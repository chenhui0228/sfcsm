...
logfile = "/var/log/sfcsm/cephprobe.log"
clusterName = "ceph"
fsid = ""
...
def get_radosgw_usage():
    p = subprocess.Popen(
        'radosgw-admin usage show --show-log-entries=false --format=json',
        shell=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE)
    outdata, errdata = p.communicate()
    ...
    radosgwUsage = outdata.strip()
    return radosgwUsage
                  
def process_radosgw_usage(db):
    print str(datetime.datetime.now()), "-- Pick radosgw usage"
    sys.stdout.flush()
    usages = json.loads(get_radosgw_usage())
    usage_t = {}
    usage_t['user'] = ""
    usage_t['bytes_sent'] = 0
    usage_t['bytes_received'] = 0
    usage_t['operates'] = 0
    usage_t['successful_ops'] = 0
    for u in usages['summary']:
        usage = {}
        usage['timestamp'] = int(round(time.time() * 1000))
        usage['user'] = u['user']
        usage['bytes_sent'] = u['total']['bytes_sent']
        usage_t['bytes_sent'] = usage_t['bytes_sent'] + int(usage['bytes_sent'])
        usage['bytes_received'] = u['total']['bytes_received']
        usage_t['bytes_received'] = usage_t['bytes_received'] + int(usage['bytes_received'])
        usage['operates'] = u['total']['ops']
        usage_t['operates'] = usage_t['operates'] + int(usage['operates'])
        usage['successful_ops'] = u['total']['successful_ops']
        usage_t['successful_ops'] = usage_t['successful_ops'] + int(usage['successful_ops'])
        db.radosgwusage.insert(usage)
    usage_t['timestamp'] = int(round(time.time() * 1000))
    db.radosgwusage.insert(usage_t)

# uri : /api/v0.1/status.json
def process_status(restapi, ceph_rest_api_subfolder, db, status_refresh=5):
    if not isLeader :
        return
    ...
    if r1.status != 200:
        ...
        timecheckmap = {}
        print "TEMP : get status by rest api success"
        try:
        ...
        try:
            ...
            print "TEMP : update monstat success"
        except (RuntimeError, TypeError, NameError, KeyError):
            pass
        ...
        cluster = {"_id": c_status['output']['fsid'],
                   ...
                   "osdmap_info": c_status['output']['osdmap']['osdmap'],
                   "name": clusterName, 
        ...
        print "TEMP : update cluster success"
        clusterperf = c_status['output']['pgmap']
        clusterperf['timestamp'] = int(round(time.time() * 1000))
        # save pgmap as cluster perf into db
        db.clusterperf.insert(clusterperf)
        print "TEMP : update clusterperf success"
        sys_log_collection(cluster, db, status_refresh)
        print str(datetime.datetime.now()), "-- Process Status complete"
        # return c_status['output']['fsid']
   
def get_log(nowtime, refresh):
    print "TEMP : collect sys log start"
    collect_time_stamp = int(nowtime/1000) - int(refresh) + 1
    # collect_time = datetime.datetime.fromtimestamp(collect_time_stamp)
    loginfo = ''
    count = 0
    while (loginfo is None or loginfo == '') and count < 6:
        collect_time_stamp = collect_time_stamp - 1
        collect_time = datetime.datetime.fromtimestamp(collect_time_stamp)
        if count == 5:
            collect_time = datetime.datetime.fromtimestamp(int(nowtime/1000))
        p = subprocess.Popen(
            'tail -n 30 /var/log/ceph/ceph.log | grep -B2 -A13 "' +str(collect_time)+'"',
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        outdata, errdata = p.communicate()
        if len(errdata):
            raise RuntimeError('unable to get conf option')
        loginfo = outdata.strip()
        count = count + 1
    if loginfo == '' or loginfo is None:
        loginfo == 'log info can not collect'
    return loginfo

def sys_log_collection(cluster, db, refresh_time):
    c_health = cluster['health']
    if c_health != 'HEALTH_OK':
        syslog = {}
        syslog['description'] = ''
        count = 1
        h_len = len(cluster['health_summary'])
        # pg tatal number
        pg_num = cluster['pgmap']['num_pgs']
        # active+clean pg num
        pg_num_ac = 0
        # problem is not handled ? 0:not handle;1:handled
        pending_record = db.syslog.find({'handle_status': 0})
        # print("===============%s ==== %s"%(pending_record,pending_record.count()))
        for ps in cluster['pgmap']['pgs_by_state']:
            if ps['state_name'] == 'active+clean':
                pg_num_ac = ps['count']
        for s in cluster['health_summary']:
            if count == h_len:
                if count == 1:
                    syslog['description'] = syslog['description'] + str(s['summary'])
                else:
                    syslog['description'] = str(count) + '. '+syslog['description'] + str(s['summary'])
            else:
                syslog['description'] = str(count) + '. ' + syslog['description'] + str(s['summary']) + '; '
            count = count + 1
        # 0为未处理状态
        syslog['handle_status'] = 0
        syslog['handler'] = 'NA'
        syslog['handle_time'] = -1
        # syslog['first_occurs_time'] = -1
        # syslog['occurs_time'] = syslog['first_occurs_time']
        if c_health == 'HEALTH_WARN':
            if pg_num == pg_num_ac:
                # cluster is warn but pg is ok
                syslog['log_level'] = 1
            else:
                # some pg is not ok
                syslog['log_level'] = 2
        ...
        syslog['info'] = get_log(nowtime, refresh_time)
        print "TEMP : collect sys log end"
        if pending_record.count() != 0:
            syslog['occurs_time'] = nowtime - int(refresh_time*1000)
            syslog['first_occurs_time'] = pending_record[0]['first_occurs_time']
        else:
            syslog['first_occurs_time'] = nowtime - int(refresh_time*1000)
            syslog['occurs_time'] = syslog['first_occurs_time']
        db.syslog.update({'handle_status': syslog['handle_status']}, syslog, upsert=True)
        print "TEMP : update syslog success"
    else:
        db.syslog.update({'handle_status': 0}, {'handle_status': 1, 'handler': 'SYS', 'handle_time':int(time.time()*1000)})
        print "TEMP : update syslog success =="
...
def process_osd_dump(restapi, ceph_rest_api_subfolder, db):
    ...
    if r1.status != 200:
        ...
        for pool in pools:
            ...
            if p['auid'] : 
                p['auid'] = str(p['auid'])
            # if db.pools.find({"pool_name":p['pool_name']}):
            #     db.pools.remove({"pool_name":p['pool_name']})
...
class CephProbeDaemon(Daemon):
    ...
    def run(self):
        ...
        status_refresh = conf.get("status_refresh", 5)
        ...
        radosgw_usage_refresh = conf.get("radosgw_usage_refresh", 60)
        print "radosgw_usage_refresh = ", radosgw_usage_refresh

        radosgw_usage_window = conf.get("radosgw_usage_window", 3456000)
        print "radosgw_usage_window = ", radosgw_usage_window

        cluster_window = conf.get("cluster_window", 1296000)
        print "cluster_window = ", cluster_window

        clusterperf_window = conf.get("clusterperf_window", 1296000)
        print "clusterperf_window = ", clusterperf_window

        syslog_window = conf.get("syslog_windowsyslog_window", 1296000)
        print "syslog_window = ", syslog_window
        ...
        if status_refresh > 0:
            restapi = httplib.HTTPConnection(ceph_rest_api)
            status_thread = Repeater(evt, process_status, [restapi, ceph_rest_api_subfolder, db, status_refresh], status_refresh)
            status_thread.start()
        ...
        radosgw_usage_thread = None
        if radosgw_usage_refresh > 0:
            # restapi = httplib.HTTPConnection(ceph_rest_api)
            radosgw_usage_thread = Repeater(evt, process_radosgw_usage, [db], osd_dump_refresh)
            radosgw_usage_thread.start()
        ...
        if clusterperf_window > 0:
            clusterperf_db_drop_thread = Repeater(evt, drop_stat, [db, "clusterperf", clusterperf_window], clusterperf_window)
            clusterperf_db_drop_thread.start()

        if syslog_window > 0:
            syslog_db_drop_thread = Repeater(evt, drop_stat, [db, "syslog", syslog_window], syslog_window)
            syslog_db_drop_thread.start()
