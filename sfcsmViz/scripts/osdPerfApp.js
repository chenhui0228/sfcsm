...
var osdPerfApp = angular.module('OsdPerf', ['ngRoute','ngTable','D3Directives','ui.bootstrap','dialogs','SfcsmCommons'])
...
function OsdPerfCtrl($rootScope, $scope, $routeParams, $http, $filter, ngTableParams, $window) {
    ...
    function getOsdInfo(osdId) {
        ...
        $http({method: "post", url: sfcsmCtrlURL + $rootScope.fsid+"/osd?depth=2", data:requestData})
            .success(function (data, status) {
                $scope.osd = data[0];
                ...
                $http({method: "post", url:  sfcsmCtrlURL + $rootScope.fsid+"/osdstat", data: requestData}).
                    success(function(data,status){
                        addStatGraph(data);
                    }).
