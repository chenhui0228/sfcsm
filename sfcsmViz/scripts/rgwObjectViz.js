 ...
 */
var RGWobjectVizApp = angular.module('RGWobjectVizApp', ['ui.bootstrap','SfcsmCommons'])
...
RGWobjectVizApp.controller("RGWobjectVizCtrl", function ($scope, $http, $location, $dialogs) {
    ...
    $scope.getObjects = function(bucketName) {
        ...
        $http({method: "get", url: sfcsmCtrlURL + "S3/bucket/"+bucketName+"/list" }).
            success(function (data, status) {
    ...
    $scope.showObject = function() {
        ...
        $http({method: "get", url: sfcsmCtrlURL + "S3/object?"+ params , headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function (data, status) {
                idle();
                var nodes = [];
