...
angular.module('imageApp', ['ngRoute','ui.bootstrap','SfcsmCommons'])
...
function refreshImages(url, $http, $scope , $window) {
    $http({method: "get", url: sfcsmCtrlURL + "RBD/images"}).
...
function DetailCtrl($rootScope,$scope, $http, $routeParams, $window, $dialogs) {
    ...
    var uri = sfcsmCtrlURL + "RBD/images/"+$scope.poolName +"/"+$scope.imageName ;
    if (typeof $scope.snapName !== 'undefined')
        uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName ;
    ...
    $scope.showPoolDetail = function (poolName) {
        ...
        if (poolNum!='') $window.location.assign('/sfcsmViz/poolManagement.html#/detail/'+poolNum);
    }
    ...
    $scope.flattenImage = function(){
        $scope.uri = sfcsmCtrlURL + "RBD/images/"+$scope.poolName +"/"+$scope.imageName  +"/flatten" ;
     ...
     $scope.purgeImage = function(){
        $scope.uri = sfcsmCtrlURL + "RBD/images/"+$scope.poolName +"/"+$scope.imageName  +"/purge" ;
        $http({method: "post", url: $scope.uri }).
            success(function (data, status) {
    ...
    $scope.copyImage = function () {
        $scope.uri = sfcsmCtrlURL + "RBD/images/"+$scope.poolName +"/"+$scope.imageName  +"/copy" ;
    ...
    $scope.deleteSnapshot = function () {
        if ($scope.detailedSnap.protected=='true') return;
        $scope.uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName  ;
    ...
    $scope.rollbackSnapshot = function () {
        $scope.uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName +"/rollback"  ;
        $http({method: "post", url: $scope.uri }).
            success(function (data, status) {
    ...
    $scope.protectSnapshot = function () {
        ...
        if ($scope.detailedSnap.protected=='true') return;
        $scope.uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName +"/protect" ;
        $http({method: "post", url: $scope.uri }).
            success(function (data, status) {
    ...
    $scope.unprotectSnapshot = function () {
        ...
        $scope.uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName +"/unprotect" ;
        $http({method: "post", url: $scope.uri }).
            success(function (data, status) {
    ...
    $scope.cloneImageSnapshot = function () {
        $scope.uri = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName +"/"+$scope.imageName  +"/"+$scope.snapName +"/clone" ;
    ...
    $scope.resize = function () {
        var url = sfcsmCtrlURL + "RBD/images/"+$scope.poolName+"/"+$scope.imageName +"/resize";
...
function DeleteCtrl($rootScope, $scope, $http, $routeParams, $window, $dialogs) {
    ...
    $scope.deleteImage = function () {
        var url = sfcsmCtrlURL + "RBD/images/"+$routeParams.poolName +"/"+$routeParams.imageName  ;
...
function CreateCtrl($rootScope, $scope, $routeParams, $location, $http, $dialogs, $window) {
    ...
    $scope.create = function () {
        var url = sfcsmCtrlURL + "RBD/images/"+$scope.image.pool.poolname+"/"+$scope.image.name;
...
function CreateSnapshotCtrl($rootScope, $scope, $routeParams, $location, $http, $dialogs, $window) {
    ...
    $scope.createSnapshot = function () {
        var url = sfcsmCtrlURL + "RBD/snapshots/"+$scope.poolName+"/"+$scope.imageName+"/"+$scope.snapName;
