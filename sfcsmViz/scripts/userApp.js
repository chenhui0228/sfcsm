...
angular.module('userApp', ['ngRoute','ngSanitize','SfcsmCommons'])
...
function refreshUsers($http, $scope, $templateCache) {
    $http({method: "get", url: sfcsmCtrlURL + "S3/user", cache: $templateCache}).
...
function DetailCtrl($rootScope,$scope, $http, $routeParams, $route, $dialogs) {
    var uri = sfcsmCtrlURL + "S3/user/"+$routeParams.uid;
    ...
    $scope.removeKey = function(key){
        $scope.uri = sfcsmCtrlURL+"S3/user/"+encodeURIComponent($rootScope.detailedUser.user_id)+"/key/"+encodeURIComponent(key);
    ...
    $scope.removeSwiftKey = function(subuser, key){
        $scope.uri = sfcsmCtrlURL+"S3/user/"+$rootScope.detailedUser.user_id+"/subuser/"+encodeURIComponent(subuser)+"/key";
    ...
    $scope.removeSubuser = function(subuser){
        $scope.uri = sfcsmCtrlURL+"S3/user/"+$rootScope.detailedUser.user_id+"/subuser/"+encodeURIComponent(subuser);
...
function CreateSubuserCtrl($rootScope, $scope, $location, $http, $dialogs, $route, $routeParams) {
    ...
    $scope.submit = function () {
        ...
        $scope.uri = sfcsmCtrlURL+"S3/user/"+$scope.subuser.uid+"/subuser";
...
function CreateSwiftKeyCtrl($rootScope, $scope, $location, $http, $dialogs, $route, $routeParams) {
    ...
    $scope.submit = function () {
        $scope.code = "";
        $scope.response = "";
        $scope.uri = sfcsmCtrlURL+"S3/user/"+$scope.master.uid+"/subuser/"+$scope.master.subuser+"/key";
...
function ModifyCtrl($rootScope, $scope, $routeParams, $location, $http, $dialogs) {
    ...
    // init
    $scope.uri = sfcsmCtrlURL + "S3/user/"+$routeParams.uid ;
...
function CapabilitiesCtrl($rootScope,$scope, $http, $routeParams, $route, $dialogs, $location) {
    ...
    $scope.saveCapability = function (type, perm) {
        ...
        $scope.uri = sfcsmCtrlURL + "S3/user/"+$routeParams.uid+"/caps" ;
