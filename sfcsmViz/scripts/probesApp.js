...
var ProbesApp = angular.module('ProbesApp', ['SfcsmCommons'])
...
ProbesApp.controller("ProbesCtrl", function ($rootScope, $scope, $http) {
    ...
    function getCephProbes() {
        ...
        $http({method: "get", url: sfcsmCtrlURL + $rootScope.fsid+"/cephprobe"}).

            success(function (data,status,headers) {
                $scope.cephprobes = data;
                $scope.timestamp_sfcsm = new Date(parseInt(headers().timestamp));
    ...
    function getSysProbes() {
        ...
        $http({method: "get", url: sfcsmCtrlURL + $rootScope.fsid+"/sysprobe"}).
