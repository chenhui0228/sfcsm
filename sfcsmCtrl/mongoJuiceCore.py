...
def listObjects(db, filters, collection, depth ):
    ...
    # objs = list(db[collection].find(select, template, skip, limit,
    #              timeout, snapshot, tailable, sort))
    print select
    if sort is None:
        if limit is None:
            objs = list(db[collection].find(select, template))
        else:
            objs = list(db[collection].find(select, template).limit(limit))
    else:
        if limit is None:
            objs = list(db[collection].find(select, template).sort(sort["sortkey"], sort["direction"]))
        else:
            objs = list(db[collection].find(select, template).limit(limit).sort(sort["sortkey"], sort["direction"]))
