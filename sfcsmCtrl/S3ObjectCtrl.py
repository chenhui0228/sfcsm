...
class S3ObjectCtrl:
    ...
    def __init__(self, conf):
        ...
        self.sfcsmCtrlUrl = "http://"+conf.get("sfcsm_root", "")+"/sfcsmCtrl/"
...
# The information of the osd is retrieved thanks to mongoDB sfcsmCtrl/{clusterName}/osd?depth=2 REST URI

    def getOsdDump(self):
        Log.debug("___getOsdDump()")
        # print str(datetime.datetime.now()), "-- Process OSDDump"
        cephRestUrl = self.sfcsmCtrlUrl + self.cluster.get_fsid() + '/osd?depth=2'
        print(cephRestUrl)
